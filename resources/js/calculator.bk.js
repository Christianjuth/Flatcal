...
function posNeg() {
    if(opp == ""){
        ...
        else if(first.indexOf("-") == -1) first = "-" + first;
    ...
    if(opp != ""){
        ...
        else if(second.indexOf("-") == -1) second = "-" + second;
        else second = second.replace(/-/,"");
        inputNumber(second);
    }
...
function inputNumber(originalNumber, stripZeros) {
    if(stripZeros == undefined || stripZeros == "undefined") stripZeros = true; //default value
    var originalNumber = String(originalNumber); //parse input number as string
    var number = "";
    var valid = (originalNumber != "" && originalNumber != undefined && originalNumber != "undefined"); //validate number
    var maxLength = Math.min(Math.round(($("#input-container").width() / 18) - 1), 16); //get max legnth of input

    if(valid){
        var finalNumber = originalNumber.split('.'); //split overall so we can work with the desemel and whole number separately
        if(finalNumber[0] != "-0") finalNumber[0] = parseInt(finalNumber[0]); //parse int if not -0
